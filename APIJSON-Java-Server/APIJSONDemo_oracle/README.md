...
static { //注册权限
		...
		ACCESS_MAP.put(Users.class.getSimpleName(), getAccessMap(Users.class.getAnnotation(MethodAccess.class)));
	}
```
...
看来是 fastjson 不支持 CLOB 类型，可以在 [AbstractSQLExecutor](https://github.com/TommyLemon/APIJSON/blob/98d625ca40863a1811b62765fa1a1f9dbf1abe8b/APIJSON-Java-Server/APIJSONLibrary/src/main/java/zuo/biao/apijson/server/AbstractSQLExecutor.java).onPutColumn 里面，
...
修改`AbstractSQLConfig`，785行

```java
LIMIT count OFFSET page*count  
改成 getWhereString 后面加  row_num > page*count AND row_num <= (page + 1)*count
```

Oracle 想要分页，需要子查询，`getConditionString`
